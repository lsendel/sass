...
public class PaymentService {
    ...
    public PaymentIntent createPaymentIntent(UUID organizationId, Money amount, String currency, String description, Map<String, String> metadata) throws StripeException {
        ...
        auditService.logPaymentEvent("PAYMENT_INTENT_CREATED",
            stripePaymentIntent.getId(),
            "Payment intent created",
            Map.of("amount", amount.getAmount().toString(),
                   "currency", currency,
        ...
        logger.info("Created payment intent: {} for organization: {}", stripePaymentIntent.getId(), organizationId);
    ...
    public PaymentIntent confirmPaymentIntent(String paymentIntentId, String paymentMethodId) throws StripeException {
        ...
        auditService.logPaymentEvent("PAYMENT_INTENT_CONFIRMED",
            paymentIntentId,
            "Payment intent confirmed",
            Map.of("payment_method_id", paymentMethodId,
                   "status", confirmedIntent.getStatus(),
                   "amount", payment.getAmount().getAmount().toString()),
            "system",
            "PaymentService");

        logger.info("Confirmed payment intent: {} with status: {}", paymentIntentId, confirmedIntent.getStatus());
    ...
    public List<Payment> getOrganizationPaymentsByStatus(UUID organizationId, Payment.Status status) {
        validateOrganizationAccess(organizationId);
        return paymentRepository.findByOrganizationIdAndStatusOrderByCreatedAtDesc(organizationId, status);
    }
    ...
    public PaymentStatistics getPaymentStatistics(UUID organizationId) {
        ...
        Long totalAmountCents = paymentRepository.sumSuccessfulPaymentAmountsByOrganization(organizationId);
        Money totalAmount = totalAmountCents != null ? new Money(BigDecimal.valueOf(totalAmountCents, 2), "USD") : Money.ZERO;

        Instant thirtyDaysAgo = Instant.now().minus(30, ChronoUnit.DAYS);
        Long recentAmountCents = paymentRepository.sumSuccessfulPaymentAmountsByOrganizationAndDateRange(
            organizationId, thirtyDaysAgo, Instant.now());
        Money recentAmount = recentAmountCents != null ? new Money(BigDecimal.valueOf(recentAmountCents, 2), "USD") : Money.ZERO;

        return new PaymentStatistics(totalPayments, totalAmount, recentAmount);
    }

    public PaymentMethod attachPaymentMethod(UUID organizationId, String stripePaymentMethodId) throws StripeException {
        validateOrganizationAccess(organizationId);
        ...
        if (stripePaymentMethod.getBillingDetails() != null) {
            com.stripe.model.PaymentMethod.BillingDetails billing = stripePaymentMethod.getBillingDetails();
            ...
            if (billing.getAddress() != null) {
                ...
                address = new PaymentMethod.BillingAddress(
            ...
            paymentMethod.updateBillingDetails(
                billing.getName(),
                billing.getEmail(),
                address
            );
        ...
        long existingCount = paymentMethodRepository.countActivePaymentMethodsByOrganization(organizationId);
        if (existingCount == 0) {
        ...
        auditService.logEvent("PAYMENT_METHOD_ATTACHED", "PAYMENT_METHOD", stripePaymentMethodId,
            "Payment method attached to organization",
            Map.of("payment_method_type", (Object) type.toString(),
                   "is_default", (Object) String.valueOf(savedPaymentMethod.isDefault()),
                   "last_four", (Object) (savedPaymentMethod.getLastFour() != null ? savedPaymentMethod.getLastFour() : "N/A")),
            null, "system", "PaymentService", null);

        logger.info("Attached payment method: {} to organization: {}", stripePaymentMethodId, organizationId);
    ...
    public void detachPaymentMethod(UUID organizationId, UUID paymentMethodId) throws StripeException {
        ...
        logger.info("Detached payment method: {} from organization: {}", paymentMethodId, organizationId);
    }
    ...
    public PaymentMethod setDefaultPaymentMethod(UUID organizationId, UUID paymentMethodId) {
        ...
        paymentMethodRepository.findByOrganizationIdAndIsDefaultTrueAndDeletedAtIsNull(organizationId)
            .ifPresent(current -> {
        ...
        logger.info("Set payment method: {} as default for organization: {}", paymentMethodId, organizationId);
        return savedPaymentMethod;
    }
    ...
    public List<PaymentMethod> getOrganizationPaymentMethods(UUID organizationId) {
        validateOrganizationAccess(organizationId);
        return paymentMethodRepository.findByOrganizationIdAndDeletedAtIsNullOrderByCreatedAtDesc(organizationId);
    }
    ...
    private void updatePaymentFromStripeIntent(Payment payment, PaymentIntent stripeIntent) {
        Payment.Status newStatus = switch (stripeIntent.getStatus()) {
    ...
    private void handlePaymentIntentSucceeded(Map<String, Object> eventData) {
        ...
        paymentRepository.findByStripePaymentIntentId(paymentIntentId)
            .ifPresent(payment -> {
                ...
                auditService.logPaymentEvent("PAYMENT_SUCCEEDED",
                    paymentIntentId,
                    "Payment succeeded via webhook",
                    Map.of("amount", payment.getAmount().getAmount().toString(),
    ...
    private void handlePaymentIntentFailed(Map<String, Object> eventData) {
        ...
        paymentRepository.findByStripePaymentIntentId(paymentIntentId)
            .ifPresent(payment -> {
                ...
                auditService.logPaymentEvent("PAYMENT_FAILED",
                    paymentIntentId,
