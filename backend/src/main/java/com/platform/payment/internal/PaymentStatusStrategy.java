...
/**
 * An abstract base class for implementing the Strategy pattern for payment status transitions.
 *
 * <p>This class defines the contract for all payment status strategies and provides common
 * dependencies, such as the {@link AuditService} and {@link ApplicationEventPublisher}, to its
 * subclasses.
 */
public abstract class PaymentStatusStrategy {
   ...
   */
  public abstract void handleStatusChange(
      Payment payment, Payment.Status newStatus, StatusChangeContext context);
   ...
   * @return {@code true} if this strategy can handle the transition, {@code false} otherwise
   */
  public abstract boolean canHandle(Payment.Status currentStatus, Payment.Status newStatus);

  /**
   * A record to hold additional context about a status change.
   *
   * @param reason a human-readable reason for the change
   * @param source the source of the change (e.g., "stripe_webhook", "manual")
   * @param additionalData a map of any additional data related to the change
   */
  public record StatusChangeContext(
      String reason, String source, Map<String, Object> additionalData) {
    ...
    public static StatusChangeContext manual(String userId, String reason) {
      return new StatusChangeContext(reason, "manual", Map.of("user_id", userId));
    }
  }
}

/** A strategy for handling successful payment transitions. */
@Component
class PaymentSucceededStrategy extends PaymentStatusStrategy {

  public PaymentSucceededStrategy(
      AuditService auditService, ApplicationEventPublisher eventPublisher) {
  ...
  public void handleStatusChange(
    ...
    logger.info(
        "Payment {} succeeded for organization {}",
        payment.getStripePaymentIntentId(),
        payment.getOrganizationId());
  }
...
/** A strategy for handling failed payment transitions. */
@Component
...
/** A strategy for handling canceled payment transitions. */
@Component
class PaymentCanceledStrategy extends PaymentStatusStrategy {

  public PaymentCanceledStrategy(
      AuditService auditService, ApplicationEventPublisher eventPublisher) {
    super(auditService, eventPublisher);
  }
  ...
  public void handleStatusChange(
    ...
    logger.info(
        "Payment {} canceled for organization {}: {}",
        payment.getStripePaymentIntentId(),
        payment.getOrganizationId(),
        context.reason());
  }
...
/** A default strategy for handling other status transitions. */
@Component
class DefaultPaymentStatusStrategy extends PaymentStatusStrategy {

  public DefaultPaymentStatusStrategy(
      AuditService auditService, ApplicationEventPublisher eventPublisher) {
    super(auditService, eventPublisher);
  }
  ...
  public void handleStatusChange(
    ...
    logger.debug(
        "Payment {} status changed to {} for organization {}",
        payment.getStripePaymentIntentId(),
        newStatus,
        payment.getOrganizationId());
  }
