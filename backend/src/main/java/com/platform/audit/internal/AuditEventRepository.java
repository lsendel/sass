...
public interface AuditEventRepository extends JpaRepository<AuditEvent, UUID> {
     ...
     */
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.action = :action AND ae.createdAt BETWEEN :startDate AND :endDate "
    ...
    /**
     * Finds all audit events related to a specific resource type within an organization.
     *
     * @param organizationId the ID of the organization
     ...
     */
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.resourceType = :resourceType "
            + "ORDER BY ae.createdAt DESC")
    List<AuditEvent> findByOrganizationIdAndResourceType(
            @Param("organizationId") UUID organizationId,
     ...
     * @return a list of matching {@link AuditEvent}s
     */
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.resourceType = :resourceType ORDER BY ae.createdAt DESC")
    List<AuditEvent> findByResourceType(
            @Param("resourceType") String resourceType);

    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.ipAddress = :ipAddress AND ae.createdAt BETWEEN :startDate AND :endDate "
            + "ORDER BY ae.createdAt DESC")
     ...
     */
    @Query("SELECT COUNT(ae) FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.actorId = :actorId AND ae.createdAt >= :since")
     ...
     */
    @Query("SELECT DISTINCT ae.resourceType FROM AuditEvent ae "
            + "WHERE ae.organizationId = :organizationId ORDER BY ae.resourceType")
     ...
     */
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND (ae.action LIKE %:searchTerm% OR ae.resourceType LIKE %:searchTerm% "
            + "OR ae.correlationId LIKE %:searchTerm%) ORDER BY ae.createdAt DESC")
    ...
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            ...
            + "AND ae.createdAt >= :since ORDER BY ae.createdAt DESC")
     ...
     */
    @Query("SELECT ae.ipAddress, COUNT(ae) as count FROM AuditEvent ae "
            + "WHERE ae.organizationId = :organizationId AND ae.action = 'LOGIN_FAILED' "
            + "AND ae.createdAt >= :since GROUP BY ae.ipAddress HAVING COUNT(ae) > :threshold")
    ...
    @Query(value = """
            ...
            WHERE organization_id = :organizationId
                AND action IN ('LOGIN_FAILED', 'UNAUTHORIZED_ACCESS',
                               'SUSPICIOUS_ACTIVITY', 'ACCOUNT_LOCKOUT')
            ...
            value =
                    """
                            SELECT
                                ...
                                (DATEDIFF('HOUR', MIN(created_at), MAX(created_at)))
            ...
            value =
                    """
                            SELECT
                                ...
                                STRING_AGG(DISTINCT CAST(details AS text), '; ') as failure_reasons
                            FROM audit_events
                            WHERE organization_id = :organizationId
    ...
    @Query(
            value =
                    """
                            ...
                            GROUP BY ae.actor_id, u.email, u.name,
                                     ae.action, ae.resource_type
    ...
    @Query(
            "SELECT ae FROM AuditEvent ae WHERE "
                    ...
                    + "(:severities IS NULL OR ae.action IN :severities) AND "
    ...
    /**
     * Updates old audit events to mark them for compression. (Note: The actual compression mechanism is not handled
     * here, this is just a placeholder for future compression feature).
     *
     * @param threshold the timestamp before which events will be marked
     * @return the number of events that could be marked as compressed
     */
    @Modifying
    @Query(
            "UPDATE AuditEvent ae SET ae.details = CONCAT(CAST(ae.details AS text), ' [COMPRESSED]') "
                    + "WHERE ae.createdAt < :threshold")
    long markEventsForCompression(@Param("threshold") Instant threshold);
    ...
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.createdAt BETWEEN :fromDate AND :toDate "
            + "ORDER BY ae.createdAt DESC")
    Page<AuditEvent> findByOrganizationIdAndCreatedAtBetweenPaginated(
            @Param("organizationId") UUID organizationId,
            @Param("fromDate") Instant fromDate,
            @Param("toDate") Instant toDate,
            Pageable pageable);
    ...
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            ...
            + "AND ae.createdAt BETWEEN :fromDate AND :toDate "
            + "ORDER BY ae.createdAt DESC")
    Page<AuditEvent> findByOrganizationIdAndActorIdAndCreatedAtBetweenPaginated(
            @Param("organizationId") UUID organizationId,
            @Param("actorId") UUID actorId,
            @Param("fromDate") Instant fromDate,
            @Param("toDate") Instant toDate,
    ...
    @Query("SELECT COUNT(ae) FROM AuditEvent ae "
            ...
            + "AND ae.createdAt BETWEEN :fromDate AND :toDate")
    long countByOrganizationIdAndCreatedAtBetweenCount(
            @Param("organizationId") UUID organizationId,
            @Param("fromDate") Instant fromDate,
            @Param("toDate") Instant toDate);
    ...
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            ...
            + "ORDER BY ae.createdAt DESC")
    Optional<AuditEvent>
            findFirstByOrganizationIdAndCreatedAtBetweenOrderByCreatedAtDesc(
            @Param("organizationId") UUID organizationId,
            @Param("fromDate") Instant fromDate,
            @Param("toDate") Instant toDate);
     ...
     * Used by AuditLogSearchService for search suggestions.
     */
    @Query("SELECT DISTINCT ae.userEmail FROM AuditEvent ae WHERE ae.organizationId = :organizationId "
            + "AND ae.userEmail IS NOT NULL "
            + "AND LOWER(ae.userEmail) LIKE LOWER(CONCAT('%', :searchTerm, '%')) "
            + "ORDER BY ae.userEmail")
    ...
    /**
     * Get resource type facets for advanced search.
     * Used by AuditLogSearchService for faceted search.
     */
    @Query("SELECT ae.resourceType as value, COUNT(ae) as count FROM AuditEvent ae "
            + "WHERE ae.organizationId = :organizationId "
            + "AND ae.createdAt BETWEEN :fromDate AND :toDate "
            + "GROUP BY ae.resourceType ORDER BY COUNT(ae) DESC")
    List<FacetResult> getResourceTypeFacets(
            @Param("organizationId") UUID organizationId,
            @Param("fromDate") Instant fromDate,
            @Param("toDate") Instant toDate);
    ...
    @Query("SELECT ae FROM AuditEvent ae WHERE ae.createdAt >= :since "
            + "ORDER BY ae.createdAt DESC")
    Page<AuditEvent> findRecentEventsPageable(@Param("since") Instant since, Pageable pageable);
