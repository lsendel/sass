...
public class UserService {
    ...
    public User createUser(String email, String name, String provider, String providerId,
        ...
        auditService.logEvent("USER_CREATION_STARTED", "USER", email, "CREATE",
            Map.of(
                "email", email,
                "name", name,
                "provider", provider,
            ...
            ), null, "system", "UserService", Map.of("action", "user_creation_started"));

        // Validate email is not already taken
        if (userRepository.existsByEmailAndDeletedAtIsNull(email)) {
            // Audit failed creation attempt
            auditService.logEvent("USER_CREATION_FAILED", "USER", email, "CREATE",
                Map.of(
                    "email", email,
                    "reason", "Email already exists",
                    "provider", provider
                ), null, "system", "UserService", Map.of("error", "duplicate_email"));

            throw new IllegalArgumentException("User with email already exists: " + email);
        }

        // Validate provider and providerId combination
        Optional<User> existingProviderUser = userRepository.findByProviderAndProviderId(provider, providerId);
        if (existingProviderUser.isPresent()) {
            ...
            auditService.logEvent("USER_CREATION_FAILED", "USER", email, "CREATE",
                ...
                ), null, "system", "UserService", Map.of("error", "duplicate_provider_id"));

            throw new IllegalArgumentException("User with provider already exists: " + provider + ":" + providerId);
        }
        ...
        if (preferences != null && !preferences.isEmpty()) {
            ...
            auditService.logEvent("USER_PREFERENCES_SET", "USER", email, "UPDATE",
                Map.of(
                    "email", email,
                    "preference_keys", preferences.keySet().toString(),
                    "preference_count", preferences.size()
                ), null, "system", "UserService", null);
        }

        User savedUser = userRepository.save(user);

        // Audit successful user creation
        auditService.logEvent("USER_CREATED", "USER", savedUser.getId().toString(), "CREATE",
            Map.of(
            ...
            Map.of(
                "user_id", savedUser.getId().toString(),
                "email", email,
                "created_at", savedUser.getCreatedAt().toString(),
                "provider", provider
            ),
            "system", "UserService", Map.of("success", "user_created"));
    ...
    public User updateProfile(UUID userId, String name, Map<String, Object> preferences) {
        ...
        auditService.logEvent("USER_PROFILE_UPDATE_STARTED", "USER", userId.toString(), "UPDATE",
            Map.of(
                "user_id", userId.toString(),
                "email", user.getEmail().getValue(),
                "old_name", oldName != null ? oldName : "",
                "new_name", name != null ? name : "",
                "preferences_updated", preferences != null && !preferences.isEmpty()
            ), null, "system", "UserService", Map.of("action", "profile_update_started"));

        user.updateProfile(name, preferences);
        User savedUser = userRepository.save(user);

        // Audit successful profile update
        auditService.logEvent("USER_PROFILE_UPDATED", "USER", userId.toString(), "UPDATE",
            Map.of(
                "user_id", userId.toString(),
                "email", user.getEmail().getValue(),
            ...
            "system", "UserService", Map.of("success", "profile_updated"));
    ...
    public void deleteUser(UUID userId) {
        ...
        auditService.logEvent("USER_DELETION_STARTED", "USER", userId.toString(), "DELETE",
            Map.of(
                "user_id", userId.toString(),
                "email", user.getEmail().getValue(),
                "name", user.getName(),
            ...
            ), null, "system", "UserService", Map.of("action", "user_deletion_started"));
        ...
        auditService.logEvent("USER_DELETED", "USER", userId.toString(), "DELETE",
            Map.of(
                "user_id", userId.toString(),
                "email", userEmail,
                "name", userName,
                ...
                "deletion_type", "soft_delete",
                "user_account_age_days", java.time.Duration.between(userCreatedAt, Instant.now()).toDays()
            ),
            Map.of(
                "user_id", userId.toString(),
            ...
            "system", "UserService", Map.of("gdpr_compliance", "user_deleted"));
    ...
    public Map<String, Long> getUserCountsByProvider() {
        ...
        return providers.stream()
            .collect(java.util.stream.Collectors.toMap(
                provider -> provider,
                provider -> userRepository.countUsersByProvider(provider)
            ));
    }
    ...
    public User restoreUser(UUID userId) {
        ...
        if (deletedUser.isEmpty()) {
            ...
            auditService.logEvent("USER_RESTORATION_FAILED", "USER", userId.toString(), "RESTORE",
                Map.of(
                    "user_id", userId.toString(),
                    "reason", "User not found"
                ), null, "system", "UserService", Map.of("error", "user_not_found"));
        ...
        if (!user.isDeleted()) {
            ...
            auditService.logEvent("USER_RESTORATION_FAILED", "USER", userId.toString(), "RESTORE",
                ...
                ), null, "system", "UserService", Map.of("error", "user_not_deleted"));
        ...
        if (userRepository.existsByEmailAndDeletedAtIsNull(user.getEmail().getValue())) {
            ...
            auditService.logEvent("USER_RESTORATION_FAILED", "USER", userId.toString(), "RESTORE",
                ...
                ), null, "system", "UserService", Map.of("error", "email_conflict"));

            throw new IllegalArgumentException("Cannot restore user - email already in use: " +
                                             user.getEmail().getValue());
        }

        // Audit user restoration attempt
        auditService.logEvent("USER_RESTORATION_STARTED", "USER", userId.toString(), "RESTORE",
            Map.of(
                "user_id", userId.toString(),
                "email", user.getEmail().getValue(),
                "name", user.getName(),
            ...
            ), null, "system", "UserService", Map.of("action", "user_restoration_started"));
        ...
        auditService.logEvent("USER_RESTORED", "USER", userId.toString(), "RESTORE",
            Map.of(
                "user_id", userId.toString(),
            ...
            Map.of(
                ...
                "deletion_duration_days", java.time.Duration.between(user.getDeletedAt(), Instant.now()).toDays()
            ),
            "system", "UserService", Map.of("gdpr_compliance", "user_restored"));
