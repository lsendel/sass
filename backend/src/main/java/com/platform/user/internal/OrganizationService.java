...
public class OrganizationService {
    ...
    public Organization createOrganization(String name, String slug, Map<String, Object> settings) {
        ...
        auditService.logEvent("ORGANIZATION_CREATION_STARTED", "ORGANIZATION", slug, "CREATE",
            Map.of(
                "name", name,
                "slug", slug,
                "creator_user_id", currentUserId.toString(),
                "has_settings", settings != null && !settings.isEmpty()
            ), null, "system", "OrganizationService", Map.of("action", "organization_creation_started"));

        // Validate slug is unique
        if (organizationRepository.existsBySlugAndDeletedAtIsNull(slug)) {
            // Audit failed creation attempt
            auditService.logEvent("ORGANIZATION_CREATION_FAILED", "ORGANIZATION", slug, "CREATE",
                Map.of(
                    "name", name,
                    "slug", slug,
                    "creator_user_id", currentUserId.toString(),
                    "reason", "Organization slug already exists"
                ), null, "system", "OrganizationService", Map.of("error", "duplicate_slug"));
        ...
        OrganizationMember ownerMember = OrganizationMember.createOwner(currentUserId, savedOrganization.getId());
        memberRepository.save(ownerMember);

        // Audit successful organization creation
        auditService.logEvent("ORGANIZATION_CREATED", "ORGANIZATION", savedOrganization.getId().toString(), "CREATE",
            Map.of(
                ...
                "creator_user_id", currentUserId.toString(),
                "has_settings", settings != null && !settings.isEmpty(),
                "owner_member_id", ownerMember.getId().toString()
            ),
            Map.of(
                "organization_id", savedOrganization.getId().toString(),
            ...
            "system", "OrganizationService", Map.of("success", "organization_created"));

        logger.info("Created organization: {} with owner: {}", savedOrganization.getId(), currentUserId);
    ...
    public List<Organization> getUserOrganizations() {
        ...
        return memberRepository.findOrganizationsForUser(currentUserId)
                .stream()
     ...
     */
    public Organization updateOrganization(UUID organizationId, String name, Map<String, Object> settings) {
        ...
        auditService.logEvent("ORGANIZATION_UPDATE_STARTED", "ORGANIZATION", organizationId.toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "old_name", oldName != null ? oldName : "",
                "new_name", name != null ? name : "",
                "settings_updated", settings != null && !settings.isEmpty(),
                "admin_user_id", TenantContext.getCurrentUserId().toString()
            ), null, "system", "OrganizationService", Map.of("action", "organization_update_started"));

        organization.updateDetails(name, settings);
        Organization savedOrganization = organizationRepository.save(organization);

        // Audit successful organization update
        auditService.logEvent("ORGANIZATION_UPDATED", "ORGANIZATION", organizationId.toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            "system", "OrganizationService", Map.of("success", "organization_updated"));
    ...
    public void deleteOrganization(UUID organizationId) {
        ...
        auditService.logEvent("ORGANIZATION_DELETION_STARTED", "ORGANIZATION", organizationId.toString(), "DELETE",
            Map.of(
                "organization_id", organizationId.toString(),
                ...
                "owner_user_id", currentUserId.toString()
            ), null, "system", "OrganizationService", Map.of("action", "organization_deletion_started"));
        ...
        auditService.logEvent("ORGANIZATION_DELETED", "ORGANIZATION", organizationId.toString(), "DELETE",
            Map.of(
                "organization_id", organizationId.toString(),
                "name", orgName,
                "slug", orgSlug,
                ...
                "deletion_type", "soft_delete",
                "owner_user_id", currentUserId.toString(),
                "organization_age_days", java.time.Duration.between(orgCreatedAt, Instant.now()).toDays()
            ),
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            "system", "OrganizationService", Map.of("gdpr_compliance", "organization_deleted"));
    ...
    public List<OrganizationMemberInfo> getOrganizationMembers(UUID organizationId) {
        ...
        return memberRepository.findMemberInfoByOrganization(organizationId)
                .stream()
                .map(member -> {
                    Optional<User> user = userRepository.findById(member.getUserId());
                    return user.map(u -> new OrganizationMemberInfo(
                            u.getId(),
                            u.getEmail().getValue(),
                            u.getName(),
                            member.getRole(),
    ...
    public Invitation inviteUser(UUID organizationId, String email, OrganizationMember.Role role) {
        ...
        auditService.logEvent("ORGANIZATION_INVITATION_STARTED", "ORGANIZATION", organizationId.toString(), "INVITE",
            Map.of(
                "organization_id", organizationId.toString(),
                "invitee_email", email,
                "invited_role", role.toString(),
                "inviter_user_id", currentUserId.toString()
            ), null, "system", "OrganizationService", Map.of("action", "invitation_started"));

        // Check if user is already a member
        Optional<User> existingUser = userService.findByEmail(email);
        if (existingUser.isPresent()) {
            boolean isAlreadyMember = memberRepository.existsByUserIdAndOrganizationId(
                existingUser.get().getId(), organizationId);
            if (isAlreadyMember) {
                // Audit failed invitation attempt
                auditService.logEvent("ORGANIZATION_INVITATION_FAILED", "ORGANIZATION", organizationId.toString(), "INVITE",
                    Map.of(
                        "organization_id", organizationId.toString(),
                        "invitee_email", email,
                        "existing_user_id", existingUser.get().getId().toString(),
                        "reason", "User is already a member"
                    ), null, "system", "OrganizationService", Map.of("error", "already_member"));
        ...
        Optional<Invitation> existingInvitation = invitationRepository
            .findPendingInvitation(organizationId, email);
        if (existingInvitation.isPresent()) {
            ...
            auditService.logEvent("ORGANIZATION_INVITATION_FAILED", "ORGANIZATION", organizationId.toString(), "INVITE",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "invitee_email", email,
                    "existing_invitation_id", existingInvitation.get().getId().toString(),
                    "reason", "Pending invitation already exists"
                ), null, "system", "OrganizationService", Map.of("error", "pending_invitation_exists"));
        ...
        auditService.logEvent("ORGANIZATION_INVITATION_CREATED", "ORGANIZATION", organizationId.toString(), "INVITE",
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            "system", "OrganizationService", Map.of("success", "invitation_created"));
    ...
    public OrganizationMember acceptInvitation(String token) {
        ...
        auditService.logEvent("ORGANIZATION_INVITATION_ACCEPTANCE_STARTED", "ORGANIZATION", "invitation", "ACCEPT",
            Map.of(
                "invitation_token", token.substring(0, Math.min(token.length(), 8)) + "...", // Partial token for security
                "accepting_user_id", currentUserId.toString()
            ), null, "system", "OrganizationService", Map.of("action", "invitation_acceptance_started"));

        Invitation invitation = invitationRepository.findByTokenAndStatus(token, Invitation.Status.PENDING)
            .orElseThrow(() -> {
                // Audit failed invitation acceptance
                auditService.logEvent("ORGANIZATION_INVITATION_ACCEPTANCE_FAILED", "ORGANIZATION", "invitation", "ACCEPT",
                    Map.of(
                        "invitation_token", token.substring(0, Math.min(token.length(), 8)) + "...",
                        "accepting_user_id", currentUserId.toString(),
                        "reason", "Invalid or expired invitation token"
                    ), null, "system", "OrganizationService", Map.of("error", "invalid_token"));
        ...
        if (invitation.isExpired()) {
            ...
            auditService.logEvent("ORGANIZATION_INVITATION_ACCEPTANCE_FAILED", "ORGANIZATION", invitation.getOrganizationId().toString(), "ACCEPT",
                Map.of(
                ...
                ), null, "system", "OrganizationService", Map.of("error", "invitation_expired"));
        ...
        if (!currentUser.getEmail().getValue().equals(invitation.getEmail())) {
            ...
            auditService.logEvent("ORGANIZATION_INVITATION_ACCEPTANCE_FAILED", "ORGANIZATION", invitation.getOrganizationId().toString(), "ACCEPT",
                ...
                ), null, "system", "OrganizationService", Map.of("error", "email_mismatch"));
        ...
        auditService.logEvent("ORGANIZATION_INVITATION_ACCEPTED", "ORGANIZATION", invitation.getOrganizationId().toString(), "ACCEPT",
            Map.of(
                "invitation_id", invitation.getId().toString(),
                "organization_id", invitation.getOrganizationId().toString(),
            ...
            "system", "OrganizationService", Map.of("success", "invitation_accepted"));

        logger.info("User {} accepted invitation and joined organization: {}",
                   currentUserId, invitation.getOrganizationId());
    ...
    public void declineInvitation(String token) {
        ...
        logger.info("Invitation declined for email: {} to organization: {}",
    ...
    public void removeMember(UUID organizationId, UUID userId) {
        ...
        auditService.logEvent("ORGANIZATION_MEMBER_REMOVAL_STARTED", "ORGANIZATION", organizationId.toString(), "REMOVE_MEMBER",
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            ), null, "system", "OrganizationService", Map.of("action", "member_removal_started"));
        ...
        if (member.isOwner()) {
            ...
            if (ownerCount <= 1) {
                ...
                auditService.logEvent("ORGANIZATION_MEMBER_REMOVAL_FAILED", "ORGANIZATION", organizationId.toString(), "REMOVE_MEMBER",
                    Map.of(
                        "organization_id", organizationId.toString(),
                        "member_user_id", userId.toString(),
                        "member_role", member.getRole().toString(),
                    ...
                    ), null, "system", "OrganizationService", Map.of("error", "last_owner_protection"));
        ...
        auditService.logEvent("ORGANIZATION_MEMBER_REMOVED", "ORGANIZATION", organizationId.toString(), "REMOVE_MEMBER",
            Map.of(
                "organization_id", organizationId.toString(),
                "removed_member_user_id", userId.toString(),
                "removed_member_role", memberRole,
                "removing_admin_user_id", currentUserId.toString(),
                "membership_duration_days", java.time.Duration.between(memberJoinedAt, Instant.now()).toDays()
            ),
            Map.of(
                "removed_at", Instant.now().toString(),
                "was_member_for_days", java.time.Duration.between(memberJoinedAt, Instant.now()).toDays()
            ),
            "system", "OrganizationService", Map.of("success", "member_removed"));
     ...
     * Update member role
     */
    public OrganizationMember updateMemberRole(UUID organizationId, UUID userId, OrganizationMember.Role newRole) {
        validateOrganizationAccess(organizationId, OrganizationMember.Role.ADMIN);
        ...
        auditService.logEvent("ORGANIZATION_MEMBER_ROLE_CHANGE_STARTED", "ORGANIZATION", organizationId.toString(), "CHANGE_ROLE",
            Map.of(
                "organization_id", organizationId.toString(),
                "member_user_id", userId.toString(),
            ...
            ), null, "system", "OrganizationService", Map.of("action", "role_change_started"));
        ...
        if (member.isOwner() && newRole != OrganizationMember.Role.OWNER) {
            ...
            if (ownerCount <= 1) {
                ...
                auditService.logEvent("ORGANIZATION_MEMBER_ROLE_CHANGE_FAILED", "ORGANIZATION", organizationId.toString(), "CHANGE_ROLE",
                    Map.of(
                        "organization_id", organizationId.toString(),
                        "member_user_id", userId.toString(),
                        "old_role", oldRole.toString(),
        ...
        auditService.logEvent("ORGANIZATION_MEMBER_ROLE_CHANGED", "ORGANIZATION", organizationId.toString(), "CHANGE_ROLE",
            Map.of(
                "organization_id", organizationId.toString(),
                "member_user_id", userId.toString(),
                "old_role", oldRole.toString(),
            ...
            "system", "OrganizationService", Map.of("success", "role_changed"));

        logger.info("Updated role for user {} in organization {} to: {}",
    ...
    private void validateOrganizationAccess(UUID organizationId, OrganizationMember.Role requiredRole) {
        Organization organization = findById(organizationId)
            .orElseThrow(() -> new IllegalArgumentException("Organization not found: " + organizationId));
        validateOrganizationAccess(organization, requiredRole);
    }

    private void validateOrganizationAccess(Organization organization, OrganizationMember.Role requiredRole) {
        UUID currentUserId = TenantContext.getCurrentUserId();
        if (currentUserId == null) {
            throw new SecurityException("Authentication required");
        }
        ...
        OrganizationMember member = memberRepository
            ...
            .orElseThrow(() -> new SecurityException("Access denied - not a member of this organization"));
    ...
    private void publishInvitationCreatedEvent(Invitation invitation, Organization organization,
                                              UUID invitedBy, OrganizationMember.Role role) {
