...
/**
 * Service for managing OAuth2 user information and provider data synchronization. Handles user
 * information lifecycle, data updates from providers, and privacy controls.
 *
 * <p>This service manages OAuth2UserInfo entities which store provider-specific user information
 * separately from the main User entities in the platform.
 */
@Service
@Transactional
public class OAuth2UserService {
  ...
  public OAuth2UserInfo findOrCreateUserInfo(
    ...
    if (existingUserInfo.isPresent()) {
      ...
      logger.info(
          "Created new OAuth2 user info: provider={}, providerUserId={}, email={}",
          provider,
          providerUserId,
          email);
  ...
  public OAuth2UserInfo updateUserInfoFromProvider(
      String providerUserId,
      String provider,
      String email,
      String name,
    ...
    OAuth2UserInfo userInfo =
        userInfoRepository
            ...
            .orElseThrow(
                () ->
                    new IllegalArgumentException("User info not found for provider: " + provider));
    ...
    logger.info(
        "Updated OAuth2 user info from provider: provider={}, providerUserId={}",
        provider,
        providerUserId);
  ...
  public void deleteUserInfo(String providerUserId, String provider, String requestedBy) {
    ...
    if (userInfoOpt.isPresent()) {
      ...
      logger.info(
          "Deleted OAuth2 user info: provider={}, providerUserId={}, requestedBy={}",
          provider,
          providerUserId,
          requestedBy);
    }
    ...
    userInfoRepository
        ...
        .ifPresent(
            userInfo -> {
              ...
              logger.debug(
                  "Updated timezone for OAuth2 user: provider={}, providerUserId={}, timezone={}",
                  provider,
                  providerUserId,
                  timezone);
            });
  ...
  public List<OAuth2UserInfo> getUsersWithUnverifiedEmails(String provider, int limit) {
    return userInfoRepository.findByProviderAndEmailVerifiedFalseOrEmailVerifiedIsNull(
        provider, limit);
  }
  ...
  private void updateUserInfo(
    ...
    logger.debug(
        "Updated OAuth2 user info: provider={}, providerUserId={}, email={}",
  ...
  public record OAuth2UserStats(long totalUsers, long verifiedEmailUsers, long usersWithPictures) {}

  /** Provider statistics record */
  public record OAuth2ProviderStats(String provider, long userCount, long verifiedEmailCount) {}
