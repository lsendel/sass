...
/**
 * Repository for OAuth2Session entities.
 * Manages OAuth2 authentication sessions.
 */
@Repository
public interface OAuth2SessionRepository extends JpaRepository<OAuth2Session, Long> {
     ...
     */
    @Query("SELECT s FROM OAuth2Session s WHERE s.isActive = true AND " +
           "s.expiresAt > :now AND s.expiresAt < :refreshThreshold")
    List<OAuth2Session> findSessionsNeedingRefresh(@Param("now") Instant now,
     ...
     */
    @Query("SELECT s.provider, COUNT(s), " +
     ...
     */
    @Query("UPDATE OAuth2Session s SET s.lastAccessedAt = :now, s.lastAccessedFromIp = :ip " +
           "WHERE s.sessionId = :sessionId")
    void updateLastAccessed(@Param("sessionId") String sessionId,
