...
/**
 * Repository for OAuth2AuditEvent entities. Manages audit trail for OAuth2 authentication and
...
public interface OAuth2AuditEventRepository extends JpaRepository<OAuth2AuditEvent, Long> {
  ...
  List<OAuth2AuditEvent> findByEventTypeOrderByEventTimestampDesc(
      OAuth2AuditEvent.OAuth2EventType eventType);
  ...
  List<OAuth2AuditEvent> findBySuccessFalseAndEventTimestampAfterOrderByEventTimestampDesc(
      Instant after);
  ...
  @Query(
      "SELECT COUNT(e) FROM OAuth2AuditEvent e WHERE e.userId = :userId AND "
  ...
  @Query(
      "SELECT COUNT(e) FROM OAuth2AuditEvent e WHERE e.ipAddress = :ipAddress AND "
  ...
  @Query(
      "SELECT e.ipAddress, e.userId, COUNT(e) as attempts "
          + "FROM OAuth2AuditEvent e "
  ...
  List<Object[]> findSuspiciousActivityPatterns(
      @Param("since") Instant since, @Param("threshold") long threshold);
  ...
  @Query(
      "SELECT e FROM OAuth2AuditEvent e WHERE e.userId = :userId AND "
