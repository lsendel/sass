...
/**
 * OAuth2 Provider entity representing configured OAuth2 authentication providers Stores provider
 * configuration and metadata for Google, GitHub, Microsoft, etc.
 *
 * <p>This entity is part of the auth module's internal domain model and should not be directly
 * exposed outside the auth module boundary.
 */
@Entity
@Table(
...
public class OAuth2Provider {
  ...
  /**
   * Provider name (google, github, microsoft, etc.) Must be lowercase and match OAuth2 registration
   * names
   */
  @NotBlank
  @Pattern(
      regexp = "^[a-z][a-z0-9_-]*$",
      message = "Provider name must be lowercase alphanumeric with hyphens/underscores")
  ...
  /** OAuth2 scopes requested from this provider Stored as comma-separated list */
  @NotEmpty
  @ElementCollection(fetch = FetchType.EAGER)
  @CollectionTable(name = "oauth2_provider_scopes", joinColumns = @JoinColumn(name = "provider_id"))
  @Column(name = "scope", length = 100)
  private List<String> scopes;

  /**
   * Client ID for this OAuth2 provider registration Should be configured via environment variables
   */
  @NotBlank
  ...
  /**
   * Attribute name in user info response that contains the user identifier Default is 'sub' for
  ...
  /**
   * Provider-specific configuration as JSON Used for additional settings that don't warrant
   * separate columns
   */
