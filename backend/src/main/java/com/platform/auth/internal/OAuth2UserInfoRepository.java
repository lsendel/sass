...
/**
 * Repository for OAuth2UserInfo entities.
 * Manages user information retrieved from OAuth2 providers.
 */
@Repository
public interface OAuth2UserInfoRepository extends JpaRepository<OAuth2UserInfo, Long> {
     ...
     */
    @Query("SELECT u FROM OAuth2UserInfo u WHERE u.lastUpdatedFromProvider < :threshold ORDER BY u.lastUpdatedFromProvider ASC")
    List<OAuth2UserInfo> findUsersNeedingRefresh(@Param("threshold") Instant threshold, Pageable pageable);
     ...
     */
    @Query("SELECT new com.platform.auth.internal.OAuth2UserService$OAuth2ProviderStats(" +
     ...
     */
    @Query("SELECT u FROM OAuth2UserInfo u WHERE " +
    ...
    default List<OAuth2UserInfo> findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase(
        ...
        return findByNameContainingIgnoreCaseOrEmailContainingIgnoreCase(nameQuery, emailQuery, Pageable.ofSize(limit));
    }
    ...
    List<OAuth2UserInfo> findByCreatedAtAfterOrderByCreatedAtDesc(@Param("since") Instant since, Pageable pageable);
     ...
     */
    @Query("SELECT u FROM OAuth2UserInfo u WHERE u.provider = :provider AND " +
     ...
     */
    default List<OAuth2UserInfo> findByProviderAndEmailVerifiedFalseOrEmailVerifiedIsNull(String provider, int limit) {
        return findByProviderAndEmailVerifiedFalseOrEmailVerifiedIsNull(provider, Pageable.ofSize(limit));
    }
