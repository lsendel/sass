...
/**
 * Service for OAuth2 audit logging and compliance tracking.
 * Provides comprehensive audit trails for all OAuth2-related activities
 * supporting GDPR compliance and forensic analysis.
 *
 * This service creates detailed audit logs for:
 * - OAuth2 authentication flows
 * - Session management events
 * - Security incidents and violations
 * - User consent and data processing activities
...
public class OAuth2AuditService {
    ...
    public OAuth2AuditService(OAuth2AuditEventRepository auditEventRepository,
     ...
     */
    public void logAuthorizationStarted(String provider, String userId, String sessionId,
        ...
        logger.info("OAuth2 authorization started: user={}, provider={}, session={}", userId, provider, sessionId);
    }

    /**
     * Log OAuth2 authorization completed successfully
     */
    public void logAuthorizationCompleted(String provider, String userId, String sessionId,
                                         String authCodeHash, String stateHash,
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 authorization completed: user={}, provider={}, duration={}ms", userId, provider, durationMs);
    }

    /**
     * Log OAuth2 authorization failure
     */
    public void logAuthorizationFailure(String provider, String errorCode, String errorMessage,
        ...
        saveAuditEvent(event);
        logger.warn("OAuth2 authorization failed: user={}, provider={}, error={}", userId, provider, errorCode);
    }
     ...
     */
    public void logTokenExchangeStarted(String provider, String userId, String authCodeHash, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
     ...
     */
    public void logTokenExchangeCompleted(String provider, String userId, String ipAddress, long durationMs) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.debug("OAuth2 token exchange completed: user={}, provider={}, duration={}ms", userId, provider, durationMs);
    }

    /**
     * Log OAuth2 token exchange failure
     */
    public void logTokenExchangeFailure(String provider, String userId, String errorCode,
        ...
        saveAuditEvent(event);
        logger.warn("OAuth2 token exchange failed: user={}, provider={}, error={}", userId, provider, errorCode);
    }

    /**
     * Log OAuth2 token validation failure
     */
    public void logTokenValidationFailure(String provider, String userId, String errorCode,
                                        String errorMessage, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.warn("OAuth2 token validation failed: user={}, provider={}, error={}", userId, provider, errorCode);
    }
     ...
     */
    public void logSessionCreated(String provider, String userId, String sessionId, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 session created: user={}, provider={}, session={}", userId, provider, sessionId);
    }

    /**
     * Log OAuth2 session renewed
     */
    public void logSessionRenewed(String provider, String userId, String sessionId, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 session renewed: user={}, provider={}, session={}", userId, provider, sessionId);
    }

    /**
     * Log OAuth2 session terminated
     */
    public void logSessionTerminated(String provider, String userId, String sessionId,
                                    String terminationReason, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 session terminated: user={}, provider={}, session={}, reason={}",
                   userId, provider, sessionId, terminationReason);
    }

    /**
     * Log multiple sessions terminated for a user
     */
    public void logUserSessionsTerminated(String provider, String userId, int sessionCount, String reason) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
            OAuth2AuditEvent.OAuth2EventType.SESSION_TERMINATED,
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 user sessions terminated: user={}, provider={}, count={}, reason={}",
                   userId, provider, sessionCount, reason);
    }

    /**
     * Log OAuth2 session expired
     */
    public void logSessionExpired(String provider, String userId, String sessionId, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 session expired: user={}, provider={}, session={}", userId, provider, sessionId);
    }
     ...
     */
    public void logUserLogin(String provider, String userId, String sessionId, String ipAddress, String userAgent) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
    ...
    public void logUserLogout(String provider, String userId, String sessionId, String ipAddress) {
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 user logout: user={}, provider={}, session={}", userId, provider, sessionId);
    }
     ...
     */
    public void logUserInfoUpdated(String provider, String userId, String changedFields, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.info("OAuth2 user info updated: user={}, provider={}, fields={}", userId, provider, changedFields);
    }

    /**
     * Log user information deleted (GDPR compliance)
     */
    public void logUserInfoDeleted(String provider, String userId, String requestedBy, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        event.setUserId(userId);
        event.setProvider(provider);
        event.setEventDetails("{\"requestedBy\":\"" + requestedBy + "\",\"reason\":\"GDPR_compliance\"}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());

        saveAuditEvent(event);
        logger.info("OAuth2 user info deleted: user={}, provider={}, requestedBy={}", userId, provider, requestedBy);
    }
     ...
     */
    public void logPkceValidationFailure(String provider, String userId, String sessionId, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.error("OAuth2 PKCE validation failed: user={}, provider={}, session={}", userId, provider, sessionId);
    }

    /**
     * Log state validation failure
     */
    public void logStateValidationFailure(String provider, String expectedState, String actualState, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        event.setProvider(provider);
        event.markAsCritical();
        event.setEventDetails("{\"expectedState\":\"" + expectedState + "\",\"actualState\":\"" + actualState + "\"}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());
     ...
     */
    public void logSuspiciousActivity(String provider, String description, String userId,
                                     String ipAddress, String userAgent) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        saveAuditEvent(event);
        logger.error("OAuth2 suspicious activity: user={}, provider={}, description={}", userId, provider, description);
    }
    ...
    public void logProviderDisabled(String provider, String disabledBy, String reason) {
        ...
        saveAuditEvent(event);
        logger.warn("OAuth2 provider disabled: provider={}, by={}, reason={}", provider, disabledBy, reason);
    }
    ...
    public void logRateLimitExceeded(String provider, String userId, String ipAddress) {
        ...
        saveAuditEvent(event);
        logger.warn("OAuth2 rate limit exceeded: user={}, provider={}, ip={}", userId, provider, ipAddress);
    }
    ...
    public List<OAuth2AuditEvent> getUserAuditEvents(String userId, Instant fromDate, Instant toDate) {
     ...
     */
    @Transactional(readOnly = true)
    public List<OAuth2AuditEvent> getProviderAuditEvents(String provider, Instant fromDate, Instant toDate) {
     ...
     */
    @Transactional(readOnly = true)
    public List<OAuth2AuditEvent> getFailedAuthenticationsByIp(String ipAddress, Instant fromDate, Instant toDate) {
        return auditEventRepository.findByIpAddressAndSuccessFalseAndEventTimestampBetweenOrderByEventTimestampDesc(
     ...
     */
    public void logGdprDataExport(String userId, String requestedBy, String dataTypes, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        event.setUserId(userId);
        event.setEventDetails("{\"requestedBy\":\"" + requestedBy + "\",\"dataTypes\":[" + dataTypes + "]}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());

        saveAuditEvent(event);
        logger.info("GDPR data export: user={}, requestedBy={}, dataTypes={}", userId, requestedBy, dataTypes);
    }

    /**
     * Log GDPR data deletion request
     */
    public void logGdprDataDeletion(String userId, String requestedBy, String deletionScope, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        event.setUserId(userId);
        event.setEventDetails("{\"requestedBy\":\"" + requestedBy + "\",\"deletionScope\":\"" + deletionScope + "\"}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());

        saveAuditEvent(event);
        logger.info("GDPR data deletion: user={}, requestedBy={}, scope={}", userId, requestedBy, deletionScope);
    }

    /**
     * Log user consent granted
     */
    public void logConsentGranted(String provider, String userId, String consentType,
                                String consentVersion, String ipAddress) {
        OAuth2AuditEvent event = new OAuth2AuditEvent(
        ...
        event.setUserId(userId);
        event.setProvider(provider);
        event.setEventDetails("{\"consentType\":\"" + consentType + "\",\"version\":\"" + consentVersion + "\"}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());

        saveAuditEvent(event);
        logger.info("Consent granted: user={}, provider={}, type={}, version={}",
                   userId, provider, consentType, consentVersion);
    }

    /**
     * Log user consent revoked
     */
    public void logConsentRevoked(String provider, String userId, String consentType,
        ...
        event.setUserId(userId);
        event.setProvider(provider);
        event.setEventDetails("{\"consentType\":\"" + consentType + "\",\"reason\":\"" + reason + "\"}");
        event.setRequestContext(ipAddress, null, generateCorrelationId());

        saveAuditEvent(event);
        logger.info("Consent revoked: user={}, provider={}, type={}, reason={}",
                   userId, provider, consentType, reason);
    }
    ...
    public int cleanupOldAuditEvents(Instant cutoffDate) {
        try {
            ...
            OAuth2AuditEvent cleanupEvent = new OAuth2AuditEvent(
                OAuth2AuditEvent.OAuth2EventType.GDPR_DATA_DELETION,
                "OAuth2 audit event retention cleanup started"
            );
            cleanupEvent.setEventDetails("{\"cutoffDate\":\"" + cutoffDate + "\",\"retentionPolicy\":true}");
