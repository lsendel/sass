...
public interface TokenMetadataRepository extends JpaRepository<TokenMetadata, UUID> {
     ...
     */
    @Query("SELECT tm FROM TokenMetadata tm WHERE tm.expiresAt BETWEEN :now AND :threshold AND tm.expiresAt > :now")
    List<TokenMetadata> findTokensExpiringSoon(@Param("now") Instant now, @Param("threshold") Instant threshold);
    ...
    List<TokenMetadata> findStaleTokens(@Param("threshold") Instant threshold, @Param("now") Instant now);

    /**
     * Revoke all tokens for a user by setting expiration to now
     */
    @Modifying
    @Query("UPDATE TokenMetadata tm SET tm.expiresAt = :now WHERE tm.userId = :userId AND tm.expiresAt > :now")
     ...
     */
    @Query("SELECT tm FROM TokenMetadata tm WHERE tm.userId = :userId AND JSON_EXTRACT(tm.metadata, '$.sessionType') = :sessionType AND tm.expiresAt > :now")
    List<TokenMetadata> findUserTokensBySessionType(@Param("userId") UUID userId,
     ...
     */
    @Query("SELECT tm FROM TokenMetadata tm WHERE JSON_EXTRACT(tm.metadata, '$.ipAddress') = :ipAddress AND tm.expiresAt > :now")
    List<TokenMetadata> findTokensByIpAddress(@Param("ipAddress") String ipAddress, @Param("now") Instant now);
    ...
    List<TokenMetadata> findTokensCreatedBetween(@Param("startTime") Instant startTime,
                                                @Param("endTime") Instant endTime);

    /**
     * Count tokens by session type
     */
    @Query("SELECT JSON_EXTRACT(tm.metadata, '$.sessionType') as sessionType, COUNT(tm) " +
