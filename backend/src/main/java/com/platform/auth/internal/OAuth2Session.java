...
/**
 * OAuth2 Session entity representing active user authentication sessions
 * Stores session metadata and links to user information from OAuth2 providers
 *
 * This entity manages the lifecycle of authenticated sessions and provides
 * secure session management with proper expiration and validation.
 */
@Entity
@Table(name = "oauth2_sessions",
...
public class OAuth2Session {
    ...
    /**
     * Unique session identifier (corresponds to HTTP session ID)
     * This is the session ID stored in the user's session cookie
     */
    @NotBlank
    ...
    /**
     * Original OAuth2 authorization code used (for audit purposes)
     * Stored in hashed form for security
     */
     ...
     */
    public OAuth2Session(String sessionId, OAuth2UserInfo userInfo, String provider, Instant expiresAt) {
