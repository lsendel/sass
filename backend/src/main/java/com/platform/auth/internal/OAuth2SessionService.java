...
/**
 * Service for managing OAuth2 sessions using OAuth2-specific entities.
 * Handles session lifecycle, user information sync, and audit logging.
 *
 * This service specifically manages OAuth2Session entities which are separate
 * from the general User/Session management in the platform.
 */
@Service
@Transactional
public class OAuth2SessionService {
    ...
    public OAuth2SessionService(OAuth2SessionRepository sessionRepository,
     ...
     */
    public OAuth2SessionResult createSession(String sessionId,
        ...
        if (email == null || providerUserId == null) {
            auditService.logAuthenticationFailure(provider, "Missing required OAuth2 attributes", ipAddress);
        ...
        try {
            ...
            logger.info("Created OAuth2 session {} for user {} via provider {}",
                       sessionId, userInfo.getProviderUserId(), provider);

            return new OAuth2SessionResult(savedSession, userInfo, true);

        } catch (Exception e) {
            auditService.logAuthenticationFailure(provider, "Session creation failed: " + e.getMessage(), ipAddress);
    ...
    public void terminateSession(String sessionId, String reason, String ipAddress) {
        Optional<OAuth2Session> sessionOpt = sessionRepository.findBySessionIdAndIsActiveTrue(sessionId);
        ...
        if (sessionOpt.isPresent()) {
            ...
            auditService.logSessionTerminated(session.getProvider(),
                                            session.getUserInfo().getProviderUserId(),
                                            sessionId, reason, ipAddress);
    ...
    public int terminateAllUserSessions(String provider, String providerUserId, String reason) {
        ...
        List<OAuth2Session> activeSessions = sessionRepository
            .findByUserInfoAndIsActiveTrueAndTerminatedAtIsNull(userInfoOpt.get());
        ...
        logger.info("Terminated {} OAuth2 sessions for user {} ({})",
                   activeSessions.size(), providerUserId, provider);
    ...
    public long getActiveSessionCountByProvider(String provider) {
        return sessionRepository.countByProviderAndIsActiveTrueAndExpiresAtAfter(provider, Instant.now());
    }

    // Private helper methods

    private OAuth2UserInfo findOrCreateUserInfo(OAuth2User oauth2User, String provider,
    ...
    private void updateUserInfoFromOAuth2User(OAuth2UserInfo userInfo, OAuth2User oauth2User) {
        ...
        userInfo.updateFromProvider(email, name, givenName, familyName,
