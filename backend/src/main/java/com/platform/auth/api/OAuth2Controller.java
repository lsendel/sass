...
/**
 * REST controller for OAuth2 authentication endpoints with PKCE support.
 * Handles OAuth2 authorization flows, provider configuration, and session management.
 */
@RestController
...
public class OAuth2Controller {
    ...
    public ResponseEntity<Map<String, Object>> initiateAuthorization(
        ...
        try {
            ...
            String authorizationUrl = buildAuthorizationUrl(clientRegistration, state, codeChallenge, redirect_uri);

            // Log authorization start
            auditService.logAuthorizationStarted(provider, null, null, ipAddress, request.getHeader("User-Agent"));
            ...
            auditService.logAuthenticationFailure(provider, "Authorization initiation failed: " + e.getMessage(), ipAddress);
    ...
    public ResponseEntity<Map<String, Object>> handleCallback(
        ...
        try {
            ...
            if (error != null) {
                ...
                return ResponseEntity.badRequest()
                    .body(Map.of(
                        ...
                        "error", Map.of(
                            "code", errorCode,
                            "message", error_description != null ? error_description : "OAuth2 authorization failed",
                            "details", Map.of(
                                "provider_error", error,
                                "provider_description", Objects.toString(error_description, "")
                            )
                        )
                    ));
            }
            ...
            OAuth2SessionService.OAuth2SessionResult sessionResult = sessionService.createSession(
            ...
            auditService.logAuthenticationFailure(provider, "Callback processing failed: " + e.getMessage(), ipAddress);
    ...
    public ResponseEntity<Map<String, Object>> logout(
        ...
        try {
            if (sessionId != null) {
                boolean terminateProviderSession = logoutRequest != null &&
                ...
                if (userPrincipal != null) {
                    auditService.logUserLogout("unknown", userPrincipal.getUserId().toString(), sessionId, ipAddress);
                }

                logger.info("OAuth2 session terminated: sessionId={}, terminateProvider={}",
                           sessionId, terminateProviderSession);
            }
    ...
    private String buildAuthorizationUrl(ClientRegistration registration, String state,
                                       String codeChallenge, String redirectUri) {
