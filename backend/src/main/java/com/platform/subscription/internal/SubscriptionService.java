...
public class SubscriptionService {
    ...
    public Subscription createSubscription(UUID organizationId, UUID planId, String paymentMethodId, boolean trialEligible) throws StripeException {
        validateOrganizationAccess(organizationId);

        // Audit subscription creation attempt
        auditService.logEvent("SUBSCRIPTION_CREATION_STARTED", "SUBSCRIPTION", "PENDING", "CREATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "plan_id", planId.toString(),
                "payment_method_id", paymentMethodId != null ? paymentMethodId : "none",
                "trial_eligible", String.valueOf(trialEligible)
            ), null, "system", "SubscriptionService", Map.of("action", "subscription_creation_started"));

        // Check if organization already has an active subscription
        Optional<Subscription> existingSubscription = subscriptionRepository.findByOrganizationId(organizationId);
        if (existingSubscription.isPresent() && existingSubscription.get().isActive()) {
            // Audit failed attempt
            auditService.logEvent("SUBSCRIPTION_CREATION_FAILED", "SUBSCRIPTION", "REJECTED", "CREATE",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "reason", "Organization already has active subscription",
                    "existing_subscription_id", existingSubscription.get().getId().toString()
                ), null, "system", "SubscriptionService", Map.of("error", "duplicate_subscription"));
        ...
        if (!plan.isActive()) {
            ...
            auditService.logEvent("SUBSCRIPTION_CREATION_FAILED", "SUBSCRIPTION", "REJECTED", "CREATE",
                ...
                ), null, "system", "SubscriptionService", Map.of("error", "inactive_plan"));

            throw new IllegalArgumentException("Plan is not active: " + planId);
        }

        // Audit plan selection
        auditService.logEvent("SUBSCRIPTION_PLAN_SELECTED", "SUBSCRIPTION", planId.toString(), "SELECT",
            Map.of(
                "organization_id", organizationId.toString(),
                "plan_id", planId.toString(),
                "plan_name", plan.getName(),
                "plan_price", plan.getPrice().toString()
            ), null, "system", "SubscriptionService", null);

        // Create or get Stripe customer
        String customerId = getOrCreateStripeCustomer(organization);

        // Audit Stripe customer creation/retrieval
        auditService.logEvent("STRIPE_CUSTOMER_ASSOCIATED", "SUBSCRIPTION", customerId, "ASSOCIATE",
            Map.of(
                "organization_id", organizationId.toString(),
        ...
        if (paymentMethodId != null) {
            ...
            auditService.logEvent("PAYMENT_METHOD_ATTACHED", "SUBSCRIPTION", paymentMethodId, "ATTACH",
                Map.of(
                    "organization_id", organizationId.toString(),
        ...
        if (trialEligible && plan.getTrialDays() != null && plan.getTrialDays() > 0) {
            ...
            auditService.logEvent("SUBSCRIPTION_TRIAL_ASSIGNED", "SUBSCRIPTION", "TRIAL", "ASSIGN",
                Map.of(
                    "organization_id", organizationId.toString(),
        ...
        auditService.logEvent("STRIPE_SUBSCRIPTION_CREATED", "SUBSCRIPTION", stripeSubscription.getId(), "CREATE",
            Map.of(
                "organization_id", organizationId.toString(),
        ...
        Subscription subscription = new Subscription(
            organizationId,
            planId,
            Subscription.Status.fromString(stripeSubscription.getStatus())
        );
        ...
        auditService.logEvent("SUBSCRIPTION_CREATED", "SUBSCRIPTION", savedSubscription.getId().toString(), "CREATE",
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            "system", "SubscriptionService", Map.of("success", "subscription_created"));

        logger.info("Created subscription: {} for organization: {} with plan: {}",
                   stripeSubscription.getId(), organizationId, planId);

        return savedSubscription;
    }

    public Subscription changeSubscriptionPlan(UUID organizationId, UUID newPlanId, boolean prorationBehavior) throws StripeException {
        validateOrganizationAccess(organizationId);

        Subscription subscription = subscriptionRepository.findByOrganizationId(organizationId)
            .orElseThrow(() -> new IllegalArgumentException("No active subscription found for organization: " + organizationId));

        UUID oldPlanId = subscription.getPlanId();

        // Audit plan change attempt
        auditService.logEvent("SUBSCRIPTION_PLAN_CHANGE_STARTED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
            ...
            ), null, "system", "SubscriptionService", Map.of("action", "plan_change_started"));

        if (!subscription.isActive()) {
            // Audit failed attempt
            auditService.logEvent("SUBSCRIPTION_PLAN_CHANGE_FAILED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "subscription_id", subscription.getId().toString(),
                    "reason", "Subscription is not active",
                    "current_status", subscription.getStatus().toString()
                ), null, "system", "SubscriptionService", Map.of("error", "inactive_subscription"));
        ...
        if (!newPlan.isActive()) {
            ...
            auditService.logEvent("SUBSCRIPTION_PLAN_CHANGE_FAILED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
                ...
                ), null, "system", "SubscriptionService", Map.of("error", "inactive_target_plan"));

            throw new IllegalArgumentException("Plan is not active: " + newPlanId);
        }

        // Audit target plan validation
        auditService.logEvent("SUBSCRIPTION_NEW_PLAN_VALIDATED", "SUBSCRIPTION", subscription.getId().toString(), "VALIDATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "new_plan_id", newPlanId.toString(),
        ...
        auditService.logEvent("STRIPE_SUBSCRIPTION_UPDATE_INITIATED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
        ...
        auditService.logEvent("STRIPE_SUBSCRIPTION_UPDATED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "stripe_subscription_id", subscription.getStripeSubscriptionId(),
        ...
        auditService.logEvent("SUBSCRIPTION_PLAN_CHANGED", "SUBSCRIPTION", subscription.getId().toString(), "UPDATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "old_plan_id", oldPlanId.toString(),
            ...
            "system", "SubscriptionService", Map.of("success", "plan_changed"));

        logger.info("Changed subscription plan: {} from {} to {}",
                   subscription.getId(), oldPlanId, newPlanId);

        return savedSubscription;
    }

    public Subscription cancelSubscription(UUID organizationId, boolean immediate, Instant cancelAt) throws StripeException {
        validateOrganizationAccess(organizationId);
        ...
        auditService.logEvent("SUBSCRIPTION_CANCELLATION_STARTED", "SUBSCRIPTION", subscription.getId().toString(), "CANCEL",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                ...
                "current_status", subscription.getStatus().toString()
            ), null, "system", "SubscriptionService", Map.of("action", "cancellation_started"));

        if (subscription.getStatus() == Subscription.Status.CANCELED) {
            // Audit failed attempt
            auditService.logEvent("SUBSCRIPTION_CANCELLATION_FAILED", "SUBSCRIPTION", subscription.getId().toString(), "CANCEL",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "subscription_id", subscription.getId().toString(),
                    "reason", "Subscription already canceled"
                ), null, "system", "SubscriptionService", Map.of("error", "already_canceled"));
        ...
        if (immediate) {
            ...
            auditService.logEvent("SUBSCRIPTION_IMMEDIATE_CANCELLATION", "SUBSCRIPTION", subscription.getId().toString(), "CANCEL",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "subscription_id", subscription.getId().toString(),
                    "stripe_subscription_id", subscription.getStripeSubscriptionId(),
            ...
            auditService.logEvent("STRIPE_SUBSCRIPTION_CANCELED_IMMEDIATELY", "SUBSCRIPTION", subscription.getId().toString(), "CANCEL",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "stripe_subscription_id", subscription.getStripeSubscriptionId(),
            ...
            Instant effectiveCancelAt = cancelAt != null ? cancelAt :
                subscription.getCurrentPeriodEnd().atStartOfDay(java.time.ZoneOffset.UTC).toInstant();

            // Audit scheduled cancellation
            auditService.logEvent("SUBSCRIPTION_SCHEDULED_CANCELLATION", "SUBSCRIPTION", subscription.getId().toString(), "SCHEDULE_CANCEL",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "subscription_id", subscription.getId().toString(),
                    "stripe_subscription_id", subscription.getStripeSubscriptionId(),
            ...
            auditService.logEvent("STRIPE_SUBSCRIPTION_CANCELLATION_SCHEDULED", "SUBSCRIPTION", subscription.getId().toString(), "SCHEDULE_CANCEL",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "stripe_subscription_id", subscription.getStripeSubscriptionId(),
        ...
        auditService.logEvent("SUBSCRIPTION_CANCELED", "SUBSCRIPTION", subscription.getId().toString(), "CANCEL",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "cancellation_type", immediate ? "immediate" : "scheduled",
                "final_status", savedSubscription.getStatus().toString(),
                "cancel_at", savedSubscription.getCancelAt() != null ? savedSubscription.getCancelAt().toString() : "immediate"
            ),
            Map.of(
                "subscription_id", savedSubscription.getId().toString(),
                "status", savedSubscription.getStatus().toString(),
            ...
            "system", "SubscriptionService", Map.of("success", "subscription_canceled"));

        logger.info("Canceled subscription: {} for organization: {}", subscription.getId(), organizationId);
    ...
    public Subscription reactivateSubscription(UUID organizationId) throws StripeException {
        ...
        auditService.logEvent("SUBSCRIPTION_REACTIVATION_STARTED", "SUBSCRIPTION", subscription.getId().toString(), "REACTIVATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "current_status", subscription.getStatus().toString(),
                "cancel_at", subscription.getCancelAt() != null ? subscription.getCancelAt().toString() : "none"
            ), null, "system", "SubscriptionService", Map.of("action", "reactivation_started"));

        if (subscription.getStatus() != Subscription.Status.CANCELED && subscription.getCancelAt() == null) {
            // Audit failed attempt
            auditService.logEvent("SUBSCRIPTION_REACTIVATION_FAILED", "SUBSCRIPTION", subscription.getId().toString(), "REACTIVATE",
                Map.of(
                    "organization_id", organizationId.toString(),
                    "subscription_id", subscription.getId().toString(),
                    "reason", "Subscription is not canceled or scheduled for cancellation",
                    "current_status", subscription.getStatus().toString()
                ), null, "system", "SubscriptionService", Map.of("error", "not_eligible_for_reactivation"));

            throw new IllegalStateException("Subscription is not canceled or scheduled for cancellation: " + subscription.getId());
        }

        // Audit reactivation validation
        auditService.logEvent("SUBSCRIPTION_REACTIVATION_VALIDATED", "SUBSCRIPTION", subscription.getId().toString(), "VALIDATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "current_status", subscription.getStatus().toString(),
        ...
        com.stripe.model.Subscription stripeSubscription = com.stripe.model.Subscription.retrieve(subscription.getStripeSubscriptionId());
        SubscriptionUpdateParams params = SubscriptionUpdateParams.builder()
            .putExtraParam("cancel_at", "")
            .build();

        // Audit Stripe reactivation initiation
        auditService.logEvent("STRIPE_SUBSCRIPTION_REACTIVATION_INITIATED", "SUBSCRIPTION", subscription.getId().toString(), "REACTIVATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "stripe_subscription_id", subscription.getStripeSubscriptionId(),
        ...
        auditService.logEvent("STRIPE_SUBSCRIPTION_REACTIVATED", "SUBSCRIPTION", subscription.getId().toString(), "REACTIVATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "stripe_subscription_id", subscription.getStripeSubscriptionId(),
                "new_stripe_status", updatedStripeSubscription.getStatus(),
        ...
        auditService.logEvent("SUBSCRIPTION_REACTIVATED", "SUBSCRIPTION", subscription.getId().toString(), "REACTIVATE",
            Map.of(
                "organization_id", organizationId.toString(),
                "subscription_id", subscription.getId().toString(),
                "previous_status", subscription.getStatus() == Subscription.Status.CANCELED ? "CANCELED" : "SCHEDULED_FOR_CANCELLATION",
                "new_status", savedSubscription.getStatus().toString(),
            ...
            "system", "SubscriptionService", Map.of("success", "subscription_reactivated"));

        logger.info("Reactivated subscription: {} for organization: {}", subscription.getId(), organizationId);
        return savedSubscription;
    }
    ...
    public SubscriptionStatistics getSubscriptionStatistics(UUID organizationId) {
        ...
        Money totalAmount = totalAmountCents != null ? new Money(java.math.BigDecimal.valueOf(totalAmountCents, 2), "USD") : Money.ZERO;

        Instant thirtyDaysAgo = Instant.now().minus(30, ChronoUnit.DAYS);
        Long recentAmountCents = invoiceRepository.sumPaidInvoiceAmountsByOrganizationAndDateRange(
            organizationId, thirtyDaysAgo, Instant.now());
        Money recentAmount = recentAmountCents != null ? new Money(java.math.BigDecimal.valueOf(recentAmountCents, 2), "USD") : Money.ZERO;

        return new SubscriptionStatistics(subscription.get().getStatus(), totalInvoices, totalAmount, recentAmount);
    }

    public void processWebhookEvent(String stripeEventId, String eventType, Map<String, Object> eventData) {
    ...
    private void updateSubscriptionFromStripe(Subscription subscription, com.stripe.model.Subscription stripeSubscription) {
        ...
        subscription.updateFromStripe(
            ...
            stripeSubscription.getTrialEnd() != null ? Instant.ofEpochSecond(stripeSubscription.getTrialEnd()) : null,
            stripeSubscription.getCancelAt() != null ? Instant.ofEpochSecond(stripeSubscription.getCancelAt()) : null
        );
    }
