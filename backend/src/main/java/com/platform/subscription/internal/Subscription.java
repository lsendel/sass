...
@Table(name = "subscriptions", indexes = {
    ...
    @Index(name = "idx_subscriptions_stripe", columnList = "stripe_subscription_id", unique = true),
...
public class Subscription {
    ...
    public static Subscription createActive(UUID organizationId, UUID planId,
    ...
    public int getDaysRemainingInPeriod() {
        ...
        return currentPeriodEnd.isAfter(today) ?
            (int) today.datesUntil(currentPeriodEnd).count() : 0;
    }
    ...
    public int getDaysRemainingInTrial() {
        ...
        LocalDate today = LocalDate.now();
        return trialEnd.isAfter(today) ?
            (int) today.datesUntil(trialEnd).count() : 0;
    }
    ...
    public void updateFromStripe(Status status, Instant currentPeriodStart, Instant currentPeriodEnd,
                                 Instant trialEnd, Instant cancelAt) {
        this.status = status;
        this.currentPeriodStart = currentPeriodStart != null ?
            java.time.LocalDate.ofInstant(currentPeriodStart, java.time.ZoneOffset.UTC) : null;
        this.currentPeriodEnd = currentPeriodEnd != null ?
            java.time.LocalDate.ofInstant(currentPeriodEnd, java.time.ZoneOffset.UTC) : null;
        this.trialEnd = trialEnd != null ?
            java.time.LocalDate.ofInstant(trialEnd, java.time.ZoneOffset.UTC) : null;
