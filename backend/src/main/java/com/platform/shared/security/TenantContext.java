...
/**
 * Thread-local tenant context for multi-tenant operations. Provides organization isolation at the
 * application level.
 */
public class TenantContext {
  ...
  public static <T> T executeInTenantContext(
      ...
      UUID userId,
      TenantContextCallback<T> callback) {
  ...
  /** Execute a block of code within a specific tenant context */
  public static <T> T executeInTenantContext(
      TenantInfo tenantInfo, TenantContextCallback<T> callback) {
  ...
  public static void executeInTenantContext(
      UUID organizationId, String organizationSlug, UUID userId, Runnable operation) {
